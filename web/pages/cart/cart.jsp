	...
	<div id="main">
		...
		<table>
			...
			<c:forEach items="${sessionScope.cart.items}" var="item">
				<tr>
					...
					<td>
						<input id="${item.value.id}" class="updateCount" type="text" value="${item.value.count}" style="width: 30px">
					</td>
		...
		<c:if test="${not empty sessionScope.cart.items}">
			<div class="cart_info">
				...
				<span class="cart_span"><a href="cartServlet?action=clear" id="clean">清空购物车</a></span>
				<span class="cart_span"><a href="orderServlet?action=createOrder">去结账</a></span>
			</div>
		</c:if>
		<c:if test="${ empty sessionScope.cart.items}">
			<div class="cart_info">
				当前购物车为空，快来<a href="index.jsp">选购商品</a>吧
			</div>
		</c:if>
